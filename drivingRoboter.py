...
class DrivingRoboter(RoboterController):
                ...
                while(self.avoidCloseLeftObstacle(self.limits[0]) or self.avoidCloseRightObstacle(self.limits[0])):
                    ...
                    if(self.get_distances()[1] >= self.limits[0]):
                        ...
                        while(self.searchForBarricade()):
                            ...
                            if not (any([self.get_distances()[2] <= 20 and self.get_distances()[2]!=0,
                                         self.get_distances()[1] <= 20 and self.get_distances()[1]!=0,
                                         self.get_distances()[0] <= 20 and self.get_distances()[0]!=0])):
                                self._stop()
    ...
    def anyHit(self, obstacle:Tuple[int,int,int], limit:int)->bool:
        print('ANY HIT:{}'.format(any([obstacle[0]<=limit and obstacle[0]!=0,obstacle[1]<=limit and obstacle[1]!=0,
